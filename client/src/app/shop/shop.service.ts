...
export class ShopService {
  ...
  productsCache = new Map<string, Pagination<Product[]>>();
  ...
  getProducts(useCache= true) : Observable<Pagination<Product[]>>              {
    ...
    if(this.productsCache.size > 0 && useCache) {
      if(this.productsCache.has(Object.values(this.shopParams).join('-'))){
        this.pagination = this.productsCache.get(Object.values(this.shopParams).join('-'));
    ...
    return this.http.get<Pagination<Product[]>>(this.baseUrl + 'products', {params}).pipe(
      map(response => {
        this.productsCache.set(Object.values(this.shopParams).join('-'), response);
  ...
  getProduct(id: number) {
    const product = [...this.productsCache.values()]
